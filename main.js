...
function main() {
    ...
    const computeEllipsisFS = computeEllipsisFragmentShader;
    ...
    const computeEllipsisProgram = webglUtils.createProgramFromSources(
        gl, [updatePositionVS, computeEllipsisFragmentShader]);
    ...
    const T_array = new Float32Array(
        ids.map(i => [transposed_data_array[14][i], 0, 0, 0]).flat());
    const index_array = new Float32Array(
    ...
    const TTex = createTexture(gl, T_array, particleTexWidth, particleTexHeight);
    const indexTex = createTexture(gl, index_array, 10, 4);
    ...
    const positionsEllipse = createFramebuffer(gl, indexTex);
    ...
    function render(time) {
        ...
        gl.uniform1i(updatePositionPrgLocs.TTex, 9);  // tell the shader the position texture is on texture unit 1
        ...
        { // compute ellipse points
            ...
            gl.useProgram(computeEllipsisProgram);
            gl.uniform1i(updatePositionPrgLocs.eTex, 0);  // tell the shader the position texture is on texture unit 0
            gl.uniform1i(updatePositionPrgLocs.aTex, 1);  // tell the shader the position texture is on texture unit 1
            gl.uniform1i(updatePositionPrgLocs.qTex, 2);  // tell the shader the position texture is on texture unit 1
            gl.uniform1i(updatePositionPrgLocs.iTex, 3);  // tell the shader the position texture is on texture unit 1
            ...
            gl.uniform1i(updatePositionPrgLocs.t0Tex, 8);  // tell the shader the position texture is on texture unit 1
            gl.uniform1i(updatePositionPrgLocs.TTex, 9);  // tell the shader the position texture is on texture unit 1
            gl.uniform1i(updatePositionPrgLocs.indexTex, 10);  // tell the shader the position texture is on texture unit 1
            gl.uniform2f(updatePositionPrgLocs.texDimensions, particleTexWidth, particleTexHeight);
            gl.uniform2f(updatePositionPrgLocs.canvasDimensions, gl.canvas.width, gl.canvas.height);
            gl.uniform1f(updatePositionPrgLocs.planetId, [4,4]);
